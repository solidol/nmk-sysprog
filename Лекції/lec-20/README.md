...
## Мета та задачі використання WinAPI

### Мета WinAPI

Основна мета використання WinAPI — забезпечити **прямий і повний доступ до функціональності операційної системи Windows** для розробки ефективних, швидких і керованих програм.

WinAPI дозволяє:
- працювати з ресурсами ОС без посередників
- отримати максимальну продуктивність
- реалізовувати низькорівневу логіку

---

### Основні задачі WinAPI

WinAPI використовується для вирішення наступних задач:

#### 1. Робота з файлами та введенням/виведенням
- створення, відкриття, читання, запис файлів
- робота з потоками даних
- асинхронний I/O

#### 2. Керування процесами та потоками
- створення процесів (`CreateProcess`)
- робота з потоками (`CreateThread`)
- завершення процесів

#### 3. Синхронізація
- м’ютекси (`CreateMutex`)
- семафори (`CreateSemaphore`)
- критичні секції

#### 4. Робота з пам’яттю
- виділення (`VirtualAlloc`)
- звільнення пам’яті
- керування адресним простором
...
#### 6. Мережеві можливості
- робота через WinSock
- TCP/UDP з’єднання

#### 7. Міжпроцесова взаємодія (IPC)
- канали (pipes)
- спільна пам’ять
- повідомлення
...
1. WinAPI працює безпосередньо з ядром ОС → мінімальні накладні витрати.

2. Повний контроль над ресурсами:
   - пам’яттю
   - файлами
   - потоками
   - синхронізацією

3. Доступ до всіх можливостей Windows:
   - реєстр
   - служби
   - драйвери (частково)
   - системні події

4. Підтримка асинхронності:
   - неблокуючий I/O
...
1. Складність використання
   ...
   - необхідність розуміння ОС
   - громіздкий код

2. Ручне керування ресурсами
  - Основна проблема:
  - потрібно вручну викликати `CloseHandle`
  - легко отримати витоки ресурсів

3. Низький рівень абстракції WinAPI:
  - не приховує деталей
  - вимагає роботи з:
  - хендлами
  - структурами
...
4. Висока ймовірність помилок:
   - неправильні прапорці
   - помилки доступу
   - гонки потоків (race conditions)

5. Погана переносимість WinAPI:
   - працює тільки на Windows
   - код не переноситься на Linux/macOS


## Коли варто використовувати WinAPI

- Коли потрібна максимальна продуктивність  
- Коли потрібен доступ до системних ресурсів  
- Для системного програмування  
- Для драйверів, сервісів, утиліт  



## Коли НЕ варто використовувати WinAPI

- Для простих програм  
- Коли важлива переносимість  
- Коли можна використати STL / Qt / .NET 
...
## Контрольні питання
1. Що таке хендл і чим він відрізняється від вказівника?
2. Які параметри є ключовими в CreateFile?
3. У чому різниця між OPEN_EXISTING і CREATE_ALWAYS?
4. Що таке файловий вказівник?
5. Коли потрібно використовувати блокування файлу?
6. Чому важливо викликати CloseHandle?
7. Яка різниця між синхронним та асинхронним I/O?
8. Що станеться, якщо два процеси пишуть у файл без блокування?
9. Як отримати код помилки WinAPI?
10. Чим WinAPI відрізняється від стандартної бібліотеки C?