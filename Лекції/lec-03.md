...
## Термінологія
**Скалярний тип**: тип , який містить одне значення певного діапазону. Скаляри включають арифметичні типи (цілочисленні або значення з плаваючою комою), елементи типу перерахування, типи покажчиків, типи покажчиків на члени та std::nullptr_t. Основні типи зазвичай скалярні.

**Складовий тип**: тип , який не є скалярним типом. Складові типи включають типи масивів, типи функцій, типи класів (або структури), типи об'єднання, перерахування, посилання та покажчики на нестатичні члени класу.

**Змінна**: символічна кількість даних. Ім'я можна використовувати для доступу до даних, які він посилається на область коду, де він визначений. У C++ змінна часто використовується посилання на екземпляри скалярних типів даних, а екземпляри інших типів зазвичай називаються об'єктами .

**Об'єкт**. Для простоти та узгодженості у цій статті використовується об'єкт термінів для посилання на будь-який екземпляр класу чи структури. Якщо він використовується в загальному розумінні, він включає всі типи, навіть скалярні змінні.
...
C++ - це строго типізованиймова та статично типізована мова. Кожен об'єкт має тип, і цей тип ніколи не змінюється. При оголошенні змінної коду необхідно явно вказати його тип або використовувати autoключове слово для вказівки компілятору виводити тип з ініціалізатора. При оголошенні функції в коді необхідно вказати тип значення, що повертається, і кожного аргументу. Використовуйте тип voidзначення, що повертається, якщо значення не повертається функцією. Винятком є використання шаблонів функцій, які дають змогу використовувати аргументи довільних типів.
...
![Типи даних Microsoft C++](img/lec-03/03-020.png)

Типи даних Microsoft C++

| **Тип Ім'я** | **Байт** | **Інші імена** | **Діапазон значень** |
|---|---|---|---|
| int | 4 | signed | Від −2 147 483 648 до 2 147 483 647 |
| unsigned int | 4 | unsigned | Від 0 до 4 294 967 295 |
| __int8 | 1 | char | Від -128 до 127 |
| unsigned __int8 | 1 | unsigned char | Від 0 до 255 |
| __int16 | 2 | short, short int,signed short int | Від −32 768 до 32 767 |
| unsigned __int16 | 2 | unsigned short,unsigned short int | Від 0 до 65 535 |
| __int32 | 4 | signed, signed int,int | Від −2 147 483 648 до 2 147 483 647 |
| unsigned __int32 | 4 | unsigned,unsigned int | Від 0 до 4 294 967 295 |
| __int64 | 8 | long long,signed long long | Від −9 223 372 036 854 775 808 до 9 223 372 036 854 775 807 |
| unsigned __int64 | 8 | unsigned long long | Від 0 до 18 446 744 073 709 551 615 |
| bool | 1 | ні | falseабоtrue |
| char | 1 | ні | -128–127 за замовчуванням. Від 0 до 255 за компіляції за допомогою /J |
| signed char | 1 | ні | Від -128 до 127 |
| unsigned char | 1 | ні | Від 0 до 255 |
| short | 2 | short int,signed short int | Від −32 768 до 32 767 |
| unsigned short | 2 | unsigned short int | Від 0 до 65 535 |
| long | 4 | long int,signed long int | Від −2 147 483 648 до 2 147 483 647 |
| unsigned long | 4 | unsigned long int | Від 0 до 4 294 967 295 |
| long long | 8 | ні (але еквівалентно __int64) | Від −9 223 372 036 854 775 808 до 9 223 372 036 854 775 807 |
| unsigned long long | 8 | ні (але еквівалентно unsigned __int64) | Від 0 до 18 446 744 073 709 551 615 |
| enum | Розрізняється | ні |
| float | 4 | ні | 3,4E +/- 38 (7 знаків) |
| double | 8 | ні | 1,7E +/- 308 (15 знаків) |
| long double | те саме, що іdouble | ні | Те саме, щоdouble |
| wchar_t | 2 | __wchar_t | Від 0 до 65 535 |


У таблиці наведено діапазони значень та розміри основних типів даних. Розмір однакового типу даних може відрізнятися на комп'юте­рах різних платформ, а також може залежати від застосованої операційної системи. Тому при оголошенні тієї чи іншої змінної потрібно чітко уявляти, скільки байт вона буде займати в пам'яті ЕОМ, щоб запобігти проблемам, пов'язаним з переповнен­ням і неправильною інтерпретацією даних. Діапазони кожного з типів (див. табл. 3.1) повинні бути перевірені для конкретного комп'ютера.
...
| Рядкова | Послідовність символів, що подають­ся в лапках | "sum__s=\0x5\n" |
