...
## Цикл з передумовою
В С++ існує три види циклів: while, for и do. 
...
![](img/lec-10/10-010.png)

Розглянемо ще два приклади його роботи:
...
int main()
{
    ...
    std::printf("Задумайте ціле число від 1 до 100. Комп'ютер спробує вгадати його.\n");
    std::printf("Відповідайте 'y', якщо здогадка правильна, і 'n', якщо програма помиляється.\n");

    // Перша спроба вгадати число
    std::printf("Отже, ваше число %d?\n", i);

    // Отримання відповіді
    while ((res = std::getchar()) != 'y')
    {
        if (res != '\n')
        {
            // Пропуск символу нового рядка
            std::printf("Ну тоді воно дорівнює %d\n", ++i);
        }
    }

    // Вивід повідомлення про вгадування числа
    std::printf("Число вгадано!\n");

    return 0; // Кінець програми
}
```

Та сама програма, але на базі бібліотеки iostream
...
int main()
{
    ...
    // Повідомлення для користувача
    std::cout << "Задумайте ціле число від 1 до 100. Комп'ютер спробує вгадати його.\n";
    std::cout << "Відповідайте 'y', якщо здогадка правильна, і 'n', якщо програма помиляється.\n";

    // Перша спроба вгадати число
    std::cout << "Отже, ваше число " << i << "?\n";

    // Отримання відповіді
    while ((res = std::cin.get()) != 'y')
    {
        if (res != '\n')
        {
            // Пропуск символу нового рядка
            std::cout << "Ну тоді воно дорівнює " << ++i << "\n";
        }
    }
...
int main()
{
    ...
    std::cout << "Вкажіть, будь ласка, верхню межу для пошуку простих чисел.\n";
    std::cout << "Верхня межа повинна бути 2 або більше.\n";

    std::cin >> limit;

    while (limit < 2)
    {
        // Якщо введено некоректне значення, повторити запит
        std::cout << "Ви були неуважні! Спробуйте ще раз.\n";
        std::cin >> limit;
    }

    std::cout << "Зараз надрукуємо прості числа!\n";

    for (number = 2; number <= limit; number++)
    {
        // Зовнішній цикл для перевірки простоти кожного числа
        for (divisor = 2; number % divisor != 0; divisor++)
            ;

        if (divisor == number)
        {
            // Якщо число є простим, вивести його
            std::cout << std::setw(5) << number;

            if (++count % 10 == 0)
                std::cout << "\n"; // Новий рядок кожні 10 простих чисел
        }
    }

    std::cout << "Робота завершена!\n";
